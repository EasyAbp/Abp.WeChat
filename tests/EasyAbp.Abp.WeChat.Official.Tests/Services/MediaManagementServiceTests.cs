...
public class MediaManagementServiceTests : AbpWeChatOfficialTestBase
{
    ...
    public async Task Should_Upload_Temp_Media_File_Success()
    {
        ...
        var mediaManagementWeService = await WeChatServiceFactory.CreateAsync<MediaManagementWeService>();
        var response = await mediaManagementWeService.UploadTempMediaAsync(thumbStream, MediaType.Thumb, $"{Guid.NewGuid().ToString()}.jpg");
