...
namespace EasyAbp.Abp.WeChat.Pay.Services.ProfitSharing
{
    ...
    /// 境内服务商分账相关接口的实现。
    /// </summary>
    public class ProfitSharingWeService : WeChatPayServiceBase
    {
        ...
        protected const string ProfitSharingAddReceiverUrl =
            "https://api.mch.weixin.qq.com/pay/profitsharingaddreceiver";

        protected const string ProfitSharingRemoveReceiverUrl =
        ...
        protected const string ProfitSharingReturnQueryUrl =
            "https://api.mch.weixin.qq.com/pay/profitsharingreturnquery";

        public ProfitSharingWeService(AbpWeChatPayOptions options, IAbpLazyServiceProvider lazyServiceProvider) : base(options, lazyServiceProvider)
        {
        }
        ...
        public virtual async Task<XmlDocument> ProfitSharingAsync(string mchId, string subMchId, string appId,
            string transactionId, string outOrderNo, IList<ProfitSharingReceiver> receivers, string subAppId = null)
        {
            ...
            var signStr = SignatureGenerator.Generate(request, new HMACSHA256(Encoding.UTF8.GetBytes(Options.ApiKey)),
                Options.ApiKey);
        ...
        /// <param name="subAppId">微信分配的子商户公众账号 Id，可为空。</param>
        /// <exception cref="ArgumentException">当参数校验出现错误时，会抛出本异常。</exception>
        public virtual async Task<XmlDocument> MultiProfitSharingAsync(string mchId, string subMchId, string appId,
            string transactionId, string outOrderNo, IList<ProfitSharingReceiver> receivers, string subAppId = null)
        {
            if (receivers.Count > 50)
            {
        ...
        public virtual async Task<XmlDocument> ProfitSharingQueryAsync(string mchId, string subMchId,
        ...
        public virtual async Task<XmlDocument> ProfitSharingAddReceiverAsync(string mchId, string subMchId,
            string appId, ProfitSharingReceiverDefinition newReceiver, string subAppId = null)
        {
            var request = new WeChatPayParameters();
            request.AddParameter("mch_id", mchId);
            request.AddParameter("sub_mch_id", subMchId);
        ...
        /// <param name="subAppId">微信分配的子商户公众账号 Id，可空。</param>
        public virtual async Task<XmlDocument> ProfitSharingRemoveReceiverAsync(string mchId, string subMchId,
            string appId, string receiverType, string receiverAccount, string subAppId = null)
        {
            var request = new WeChatPayParameters();
            request.AddParameter("mch_id", mchId);
            request.AddParameter("sub_mch_id", subMchId);
            ...
            request.AddParameter("receiver",
                JsonConvert.SerializeObject(new ProfitSharingReceiverDefinition(receiverType, receiverAccount, null)));
        ...
        public virtual async Task<XmlDocument> ProfitSharingFinishAsync(string mchId, string subMchId, string appId,
            string transactionId, string outOrderNo, string description)
        {
            var request = new WeChatPayParameters();
            request.AddParameter("mch_id", mchId);
            request.AddParameter("sub_mch_id", subMchId);
        ...
        /// <param name="subAppId">微信分配的子商户公众账号 Id，可空。</param>
        public virtual async Task<XmlDocument> ProfitSharingReturnAsync(string mchId, string subMchId, string appId,
            string outOrderNo, string outReturnNo, string returnAccountType, string returnAccount, int returnAmount,
            string description, string subAppId = null)
        {
            var request = new WeChatPayParameters();
            request.AddParameter("mch_id", mchId);
            request.AddParameter("sub_mch_id", subMchId);
        ...
        /// <param name="outOrderNo">原发起分账请求时使用的商户后台系统的分账单号。</param>
        /// <param name="outReturnNo">此回退单号是商户在自己后台生成的一个新的回退单号，在商户后台唯一。</param>
        public virtual async Task<XmlDocument> ProfitSharingReturnQueryAsync(string mchId, string subMchId,
            string appId, string outOrderNo, string outReturnNo)
        {
            var request = new WeChatPayParameters();
            request.AddParameter("mch_id", mchId);
            request.AddParameter("sub_mch_id", subMchId);
