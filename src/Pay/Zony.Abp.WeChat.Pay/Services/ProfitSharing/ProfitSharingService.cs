...
namespace Zony.Abp.WeChat.Pay.Services.ProfitSharing
{
    /// <summary>
    /// 境内服务商分账相关接口的实现。
    /// </summary>
    public class ProfitSharingService : WeChatPayService
    {
        ...
        /// <param name="subAppId">微信分配的子商户公众账号 Id，可为空。</param>
        /// <exception cref="ArgumentException">当参数校验出现错误时，会抛出本异常。</exception>
        public Task<XmlDocument> MultiProfitSharingAsync(string mchId, string subMchId, string appId, string transactionId, string outOrderNo,
            IList<ProfitSharingReceiver> receivers, string subAppId = null)
        {
            if (receivers.Count > 50)
            {
        ...
        public Task<XmlDocument> ProfitSharingAddReceiverAsync(string mchId, string subMchId, string appId, ProfitSharingReceiverDefinition newReceiver, string subAppId = null)
        {
            var request = new WeChatPayParameters();
            request.AddParameter("mch_id", mchId);
            request.AddParameter("sub_mch_id", subMchId);
        ...
        public Task<XmlDocument> ProfitSharingFinishAsync(string mchId, string subMchId, string appId, string transactionId, string outOrderNo, string description)
        {
            var request = new WeChatPayParameters();
            request.AddParameter("mch_id", mchId);
            request.AddParameter("sub_mch_id", subMchId);
        ...
        /// <param name="subAppId">微信分配的子商户公众账号 Id，可空。</param>
        public Task<XmlDocument> ProfitSharingReturnAsync(string mchId, string subMchId, string appId, string outOrderNo, string outReturnNo, string returnAccountType,
        ...
        /// <param name="outOrderNo">原发起分账请求时使用的商户后台系统的分账单号。</param>
        /// <param name="outReturnNo">此回退单号是商户在自己后台生成的一个新的回退单号，在商户后台唯一。</param>
        public Task<XmlDocument> ProfitSharingReturnQueryAsync(string mchId, string subMchId, string appId, string outOrderNo, string outReturnNo)
        {
            var request = new WeChatPayParameters();
            request.AddParameter("mch_id", mchId);
            request.AddParameter("sub_mch_id", subMchId);
